                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-yellow-50 text-yellow-900">
                                    <tr>
                                        ...
                                        <th class="text-center px-4 py-2">Đánh giá</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($order->orderDetails as $detail)
                                        @php
                                            $hasReviewed = $detail->product
                                        ...
                                        <tr class="border-t hover:bg-yellow-50 transition-all duration-300">
                                            ...
                                            <td class="text-center px-4 py-2">
                                                @if($order->status == 3)
                                                    @if($hasReviewed)
                                                        <button disabled
                                                            class="inline-flex items-center gap-1 bg-yellow-400 text-white px-3 py-1.5 rounded-lg text-sm font-semibold shadow cursor-not-allowed opacity-80">
                                                            🌟 Đã đánh giá
                                                        </button>
                                                    @else
                                                        <a href="{{ route('site.product_detail', $detail->product->slug) }}"
                                                           class="inline-flex items-center gap-1 bg-gradient-to-r from-green-500 to-green-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold shadow hover:scale-105 hover:shadow-green-400/60 transition">
                                                            ⭐ Đánh giá
                                                        </a>
                                                    @endif
                                                @else
                                                    <span class="text-gray-400 text-xs italic">Chưa thể đánh giá</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
